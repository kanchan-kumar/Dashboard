 ...
 * cos, sin, tan, acos, asin, atan, sqrt, sqr, log, min, max, ceil, floor, abs,
 * neg, rndr.<br>
...
public class MathEvaluator
{
  ...
  /*****************************************************************************
   * creates an empty MathEvaluator. You need to use setExpression(String s) to
  ...
  protected class Node
  {
    ...
    protected int checkBrackets(String s)
    {
      ...
      for (int i = 0; i < sLength; i++)
      {
        if (s.charAt(i) == '(' && inBracket >= 0)
          ...
          inBracket--;
        else if (s.charAt(i) == '{' && inBracket >= 0)
          inBracket++;
        else if (s.charAt(i) == '}')
          inBracket--;
        else if (s.charAt(i) == '[' && inBracket >= 0)
    ...
    public String removeBrackets(String s)
    {
      ...
      else if (s.length() > 2 && res.startsWith("{") && res.endsWith("}") && checkBrackets(s.substring(1, s.length() - 1)) == 0)
      {
        res = res.substring(1, res.length() - 1);
      }
      else if (s.length() > 2 && res.startsWith("[") && res.endsWith("]") && checkBrackets(s.substring(1, s.length() - 1)) == 0)
