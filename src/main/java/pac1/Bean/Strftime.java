 ...
 * @author Bip Thelin
 * @author Dan Sandberg
 * @version $Revision: 1.2 $, $Date: 2010/08/13 07:00:56 $
 */
public class Strftime
{
  protected static Properties translate;
  protected SimpleDateFormat simpleDateFormat;
  private ArrayList<Integer> listOfIndexForSingleQuatos = new ArrayList<Integer>();
  ...
  {
    StringBuffer format = new StringBuffer(simpleDateFormat.format(date));
    
    for (int i = 0 ; i < listOfIndexForSingleQuatos.size() ; i++)
    {
      int index = listOfIndexForSingleQuatos.get(i);
      format = format.insert(index, '\'');
    }
    ...
    for(int i = 0; i < pattern.length(); i++)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          {
            listOfIndexForSingleQuatos.add(i);
            continue;
          }
          buf.append(c);
        }
